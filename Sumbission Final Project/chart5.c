...
{//A function that creates an array of musicians for the concert as requested
	...
	while (curr != NULL && !(notFound))
	{
		...
		mergeSort(MusiciansCollection[insId].arr, MusiciansCollection[insId].logSize, importance);
...
void mergeSort(MusiciansOnIns* arr, int size, char importance)
{//Sort the array of musicians playing the instrument by price (according to the importance of the instrument)
	int* tmpArr = NULL;
	if (size <= 1)
		return;
	mergeSort(arr, size / 2, importance);
	mergeSort(arr + size / 2, size - size / 2, importance);
	tmpArr = (MusiciansOnIns*)malloc(size * sizeof(MusiciansOnIns));
	if (tmpArr)
	{
		merge(arr, size / 2, arr + size / 2, size - size / 2, tmpArr, importance);
...
void merge(MusiciansOnIns* arr1, int n1, MusiciansOnIns* arr2, int n2, MusiciansOnIns* res, char importance)
{
	int ind1, ind2, resInd;
	ind1 = ind2 = resInd = 0;
	while ((ind1 < n1) && (ind2 < n2)) {
		if (howToSort(arr1[ind1], arr2[ind2], importance))
		{
...
bool howToSort(MusiciansOnIns player1, MusiciansOnIns player2, char importance)
{//A function that determines whether we sort the array from low to high or from high to low

	if (importance == NOT_IMPORTANT)
	{
		if (player1.price <= player2.price) //If not important the low price will be first 
			return true;
		else
			return false;
	}
...
void copyArr(MusiciansOnIns* arr, MusiciansOnIns* src, int size)
{
	int i;
	for (i = 0; i < size; i++)
		arr[i] = src[i];
